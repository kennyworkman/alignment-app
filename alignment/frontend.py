"""
    frontend
    ~~~~~~~~
    A view that handles requests and renders HTML templates.
"""

from flask import (
    Blueprint, render_template, session, g, redirect
)
...
def index():
    """Processes user requests and renders templates to defined endpoints.

    This is the workhorse of application. See the `Flask Documentation
    <https://flask.palletsprojects.com/en/1.1.x/quickstart/#routing>`_ on views
    to read more.

    :return: A Flask render_template() function
    """
    ...
    # Don't want this info to stick around after the Request has been made,
    # thus g will teardown
    ...
    if align_form.align_button.data and align_form.validate_on_submit():
        gene1_name, gene1_content = align_form.gene1_name.data, \
            align_form.gene1_content.data
        gene2_name, gene2_content = align_form.gene2_name.data, \
            align_form.gene2_content.data

        g.gene_dict[gene1_name] = gene1_content
        g.gene_dict[gene2_name] = gene2_content

        # This will switch the form rendered in the template.
        # Storing this information in the session keeps it consistent
    ...
    if another_form.another_button.data:
        ...
        if another_form.validate_on_submit():
            gene_name, gene_content = another_form.gene_name.data, \
                another_form.gene_content.data
